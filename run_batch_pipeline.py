...
def main():
    ...
    for batch_start in range(0, len(binaries), BATCH_SIZE):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
            future_to_binary = {executor.submit(run_ghidra_on_binary, binary): binary for binary in current_batch_binaries}
            
            for i, future in enumerate(concurrent.futures.as_completed(future_to_binary)):
                binary_path = future_to_binary[future]
                try:
                    success, num_funcs, binary_name = future.result()
                    
                    # Calculate global index for progress
                    # Note: Completion order is not guaranteed, so index might be out of order in display
                    # but we just want to show progress
                    
                    if success:
                        results['success'] += 1
                        results['total_functions'] += num_funcs
                        safe_print(f"  {binary_name[:40]:40s} ✓ ({num_funcs} funcs)")
                    else:
                        results['failed'] += 1
                        results['failed_binaries'].append(binary_name)
                        safe_print(f"  {binary_name[:40]:40s} ✗ FAILED")
                        
                except Exception as exc:
                    binary_name = os.path.basename(binary_path)
                    results['failed'] += 1
                    results['failed_binaries'].append(binary_name)
                    safe_print(f"  {binary_name[:40]:40s} ✗ EXCEPTION: {exc}")
